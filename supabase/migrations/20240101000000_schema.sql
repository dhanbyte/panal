...
-- Step 6: RLS Policies - Allow public access for simple auth
...
CREATE POLICY "user_scores_insert" ON user_scores FOR INSERT WITH CHECK (true);
CREATE POLICY "user_scores_update" ON user_scores FOR UPDATE USING (true);

-- Migration Complete! Database ready for ShopWave Team Management App
-- Simple auth with email/password, no Supabase auth required

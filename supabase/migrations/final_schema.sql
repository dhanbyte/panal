-- Run this as ONE single query in Supabase SQL Editor
...
-- Step 2: Create users (no foreign key to auth.users — trigger se handle karenge)
CREATE TABLE users (
  id UUID PRIMARY KEY,
...
CREATE POLICY "tasks_insert" ON tasks FOR INSERT TO authenticated WITH CHECK (assigned_by = auth.uid());
...
CREATE POLICY "task_assignees_insert" ON task_assignees FOR INSERT TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM tasks WHERE id = task_assignees.task_id AND assigned_by = auth.uid()));

CREATE POLICY "task_comments_select" ON task_comments FOR SELECT TO authenticated USING (true);
CREATE POLICY "task_comments_insert" ON task_comments FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "user_scores_update" ON user_scores FOR UPDATE TO authenticated USING (user_id = auth.uid());
